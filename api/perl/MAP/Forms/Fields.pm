...
post '/'.$collectionName.'.:format' => sub {
	...
	my $strSQLtable = '
		...
		IF NOT EXISTS(SELECT * FROM sys.columns 
			WHERE [name] = N\''.$hash{name}.'\' AND [object_id] = OBJECT_ID(N\'formmaker_'.$agency_id .'_'.$formname.'_tmp\'))
		BEGIN
			ALTER TABLE dbo.formmaker_'.$agency_id .'_'.$formname.'_tmp ADD '.MAP::API->regex_alnum($hash{name}).' varchar(MAX);
		END
	';
...
del '/'.$collectionName.'/:'.$primaryKey.'.:format' => sub {
	...
	for(@IDs)
	{
		...
		my $strSQLtable = '
			...
			IF EXISTS(SELECT * FROM sys.columns 
				WHERE [name] = N\''.$field_name.'\' AND [object_id] = OBJECT_ID(N\'formmaker_'.$agency_id .'_'.$formname.'_tmp\'))
			BEGIN
				ALTER TABLE formmaker_'.$agency_id .'_'.$formname.'_tmp DROP column '.$field_name.';
			END
		';
		$sth = $dbh->prepare( $strSQLtable, );
		$sth->execute(  ) or MAP::API->fail( $sth->errstr . " --------- ".$strSQLtable );
