...
our $VERSION = '0.1';
our $collectionName = 'forms';
our $primaryKey = 'form_id';
our $tableName = 'formmaker_properties';
our $defaultColumns = 'form_id,formlabel,formname,formtext,formindex,redirecturl,adminalert,autorespond,tiplocation,display,preview,nomultiple,captcha,key_id,form_agency_id,submissionmsg,displaycolumns,numofrecords,formtype,formdisplaytype,skin';
our $root_path = '/var/www/html/userhome/MAP-API/'.$collectionName;
...
post '/'.$collectionName.'.:format' => sub {
	...
	my $strSQLtable = '
		...
		CREATE TABLE dbo.formmaker_'.$agency_id .'_'.$hash{formname}.'_tmp( 
				[data_id] INT not null Identity(1,1)
				,[user_id] INT 
				,[key_id] INT 
				,[connId] INT 
				,[connectionId] INT 
				CONSTRAINT formmaker_'.$agency_id .'_'.$hash{formname}.'_tmp_pkey PRIMARY KEY (data_id)
		);
	';
...
del '/'.$collectionName.'/:'.$primaryKey.'.:format' => sub {
	...
	for(@IDs)
	{
		...
		my $strSQLtable = '
			...
			IF OBJECT_ID(\'dbo.formmaker_'.$agency_id .'_'.$formname.'_tmp\', \'U\') IS NOT NULL
				DROP TABLE dbo.formmaker_'.$agency_id .'_'.$formname.'_tmp;
		';
		$sth = $dbh->prepare( $strSQLtable, );
		$sth->execute(  ) or MAP::API->fail( $sth->errstr . " --------- ".$strSQLtable );
	}
