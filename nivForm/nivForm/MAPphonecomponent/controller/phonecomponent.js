...
//            alert(self.window_manager);
    ...
    addRow: function (uid, task) {
        ...
        /*if(self.hasprimaryphone == 1 && (self.form[uid].getItemValue("PrimaryPhoneNum") == 0)){
            self.form[uid].setItemValue("PrimaryPhoneNum", 1);            
        }
        */
        if (self.validate(uid, task) === true) {
            self.editlayout[uid].progressOn();
            self.editstatus_bar[uid].setText("Saving Record...");
            var phonenumbr = self.form[uid].getItemValue("phoneNum");
            MAPphone_params = "contact_ID=" + self.form[uid].getItemValue("contact_ID") + "&phoneNumexten="+ self.form[uid].getItemValue("phoneNumexten") +"&PrimaryPhoneNum=" + self.form[uid].getItemValue("PrimaryPhoneNum") + "&phoneNum=" + phonenumbr + "&phoneType=" + self.form[uid].getItemValue("phoneType") + "&phone_ID=" + self.form[uid].getItemValue("phone_ID") + "&agencyid=" + self.configuration[uid].agencyid;
		...
		if(self.configuration[uid].window_managerObj !== null){
                    if(self.configuration[uid].contactcomponent_obj.configuration[uid].contactid == 0){									
                        if (self.configuration[uid].contactcomponent_obj.validation(uid) === true) {
                            self.configuration[uid].contactcomponent_obj.savecontact(uid, 'temp');	
                                ...
                                setTimeout(function(){ self._form(uid, 'New')},4000);
            ...
            if (id == "delete_phone") {
                selectedRowsId = self.grid[uid].getSelectedRowId();
                 self.progressOn(uid);
                if (self.grid[uid].cells(selectedRowsId, 3).getValue('add_primarytext') == 'Yes') {
                    self.progressOff(uid);
                    dhtmlx.message({
                        type: "error",
                        text: "Primary phone number cannot be deleted..."
                    });
                    return true;
                }
...
//                self._window_manager();
    ...
    _form: function (uid, task) {
        ...
        self.form[uid].getCombo("phoneType").enableOptionAutoHeight(1);
        self.form[uid].getCombo("phoneType").readonly(true);
        self.form[uid].attachEvent("onInputChange", function (name_input, value_input){
            self.formedited = 1;
           if (name_input == 'phoneNum') {
               
                var formatedphonenum = value_input;
                formatedphonenum = formatedphonenum.replace(/\D/g, "");
                formatedphonenum = formatedphonenum.substring(0, 10);
                formatedphonenum = formatedphonenum.replace(/^(\d{3})(\d)/g, "$1-$2");
                formatedphonenum = formatedphonenum.replace(/^(\d{3})\-(\d{3})(\d)/g, "$1-$2-$3");
                formatedphonenum = formatedphonenum.replace(/(\d)\-(\d{3})$/, "$1-$2");
                self.form[uid].setItemValue(name_input, formatedphonenum);
           }
           if (name_input == 'phoneNumexten') {
               var formatedphoneext = value_input;
              formatedphoneext = formatedphoneext.replace(/\D/g, "");
              formatedphoneext = formatedphoneext.substring(0, 5);
              self.form[uid].setItemValue(name_input, formatedphoneext);
           }
        });
        ...
        if (task == 'Edit') {
            ...
            self.form[uid].setItemValue("phone_ID", self.grid[uid].getSelectedRowId());
                ...
                var formatedphonenum = self.grid[uid].cells(self.grid[uid].getSelectedRowId(), 1).getValue();
                formatedphonenum = formatedphonenum.replace(/\D/g, "");
                formatedphonenum = formatedphonenum.substring(0, 10);
                formatedphonenum = formatedphonenum.replace(/^(\d{3})(\d)/g, "$1-$2");
                formatedphonenum = formatedphonenum.replace(/^(\d{3})\-(\d{3})(\d)/g, "$1-$2-$3");
            ...
            self.form[uid].setItemValue("phoneNum", formatedphonenum);
            //self.hasprimaryphone = primary_phone;
            ...
            self.form[uid].getCombo("phoneType").setComboText("-- Select Phone Type --");
            self.form[uid].getCombo("phoneType").loadXML(self.configuration[uid].application_path + "processors/phonetypelist.php?agencyid=" + self.configuration[uid].agencyid+"&id=0", self.progressOff(uid));
                ...
                self.form[uid].getCombo("phoneType").attachEvent("onXLE",function(){
                    self.editlayout[uid].progressOff();
                     self.editstatus_bar[uid].setText("Ready to use.");
                    self.form[uid].getCombo("phoneType").attachEvent("onChange",function(){
                        self.formedited = 1;
                    });    
                });
            self.form[uid].attachEvent("onChange", function (name,value) {
                if(name == "PrimaryPhoneNum")
                    self.formedited = 1;
            });
    },
        ...
        dhtmlxAjax.post(self.configuration[uid].application_path + "processors/get_data.php", postStr, function (loader) {
            try {
                ...
                if (json.status == "success") {
                    ...
                    self.progressOff(uid);
                } else if(json.status == "norecords"){
                        self.progressOff(uid);
                        var z=self.grid[uid].getStateOfView();
                    if(z[2] == 0){
                        self.grid[uid].addRow(-1, ["<div style='margin: 0 auto; color: #00A9E1; width:550px; font-size : 20px; text-align: center;'>No records found.Please click the 'Add Phone' option in the toolbar to add a phone</div>"]);
                        self.grid[uid].setRowTextStyle(-1, "height:320px;");
                        self.grid[uid].enableColSpan(true);
                        self.grid[uid].setColspan(-1, 0, 4);
                    }    
                }else {
                    dhtmlx.message({
                        type: "error",
