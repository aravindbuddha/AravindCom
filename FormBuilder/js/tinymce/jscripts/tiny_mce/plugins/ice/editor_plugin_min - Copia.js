		...
		init: function(ed, url) {
			...
			ed.addCommand('initializeice', function(editor) {
				...
				tinymce.DOM.win.setTimeout(function() {
					...
					window.changeEditor[ ed.id ] = new ice.InlineChangeEditor({
						element: ed.getBody(),
					...
					ed.onEvent.add(function(ed, e) {
						return window.changeEditor[ ed.id ].handleEvent(e);
					});
			...
			ed.addCommand('ice_initenv', function() {
				window.changeEditor[ ed.id ].initializeEnvironment();
				window.changeEditor[ ed.id ].initializeRange();
			});
			...
			ed.addCommand('icecleanbody', function(el) {
				var body = window.changeEditor[ ed.id ].getCleanContent(el || ed.getContent(), self.afterClean, self.beforeClean);
				return body;
			});
			...
			ed.addCommand('ice_hasDeletePlaceholders', function() {
				return window.changeEditor[ ed.id ].isPlaceholdingDeletes;
			});
			...
			ed.addCommand('ice_addDeletePlaceholders', function() {
				return window.changeEditor[ ed.id ].placeholdDeletes();
			});
			...
			ed.addCommand('ice_removeDeletePlaceholders', function() {
				return window.changeEditor[ ed.id ].revertDeletePlaceholders();
			});
			...
			ed.addCommand('iceinsert', function(insert) {
				insert = insert || {};
				window.changeEditor[ ed.id ].insert(insert.item, insert.range);
			});
			...
			ed.addCommand('icedelete', function(del) {
				del = del || {};
				window.changeEditor[ ed.id ].deleteContents(del.right, del.range);
			});
			...
			ed.addCommand('ice_changeuser', function(user) {
				window.changeEditor[ ed.id ].setCurrentUser(user);
			});
			...
			ed.addCommand('iceaccept', function(node) {
				ed.undoManager.add();
				window.changeEditor[ ed.id ].acceptChange(node || ed.selection.getNode());
				cleanup();
			});
			...
			ed.addCommand('icereject', function(node) {
				ed.undoManager.add();
				window.changeEditor[ ed.id ].rejectChange(node || ed.selection.getNode());
				cleanup();
			});
			...
			ed.addCommand('iceacceptall', function() {
				ed.undoManager.add();
				window.changeEditor[ ed.id ].acceptAll();
				cleanup();
			});
			...
			ed.addCommand('icerejectall', function() {
				ed.undoManager.add();
				window.changeEditor[ ed.id ].rejectAll();
				cleanup();
			});
			...
			ed.addCommand('ice_smartquotes', function(quiet) {
				window.changeEditor[ ed.id ].pluginsManager.plugins['IceSmartQuotesPlugin'].convert(ed.getBody());
			...
			ed.addCommand('ice_togglechanges', function() {
				if(window.changeEditor[ ed.id ].isTracking) {
			...
			ed.addCommand('ice_enable', function() {
				window.changeEditor[ ed.id ].enableChangeTracking();
				ed.controlManager.setActive('ice_togglechanges', true);
			...
			ed.addCommand('ice_disable', function() {
				window.changeEditor[ ed.id ].disableChangeTracking();
			...
			ed.addCommand('ice_isTracking', function() {
				return window.changeEditor[ ed.id ].isTracking ? 1 : 0;
			});
			...
			ed.addCommand('ice_strippaste', function(html) {
				return window.changeEditor[ ed.id ].pluginsManager.plugins['IceCopyPastePlugin'].stripPaste(html);
			});
			...
			ed.addCommand('ice_handlepaste', function(html) {
				return window.changeEditor[ ed.id ].pluginsManager.plugins['IceCopyPastePlugin'].handlePaste();
			});
			...
			ed.addCommand('ice_handleemdash', function(html) {
				return window.changeEditor[ ed.id ].pluginsManager.plugins['IceEmdashPlugin'].convertEmdash() ? 1 : 0;
			});
